export function initTruyenTheoDoi() {
    ...
    fetch('/truyenviethay/api/api.php?action=followed_truyen')
        ...
        .then(data => {
            ...
            followedList.innerHTML = data.map(truyen => {
                    ...
                    <div class="truyen-item">
                        <div class="truyen-image">
                            <a href="../truyen/chi-tiet-truyen.html?truyen_id=${truyen.id}">
                                <img src="${truyen.anh_bia_url || '/truyenviethay/anh/default.jpg'}" alt="${truyen.ten_truyen}">
                            </a>
                            <span class="truyen-status">${timeAgo}</span>
                            <button class="unfollow-btn" data-truyen-id="${truyen.id}" title="Hủy theo dõi">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                        <h3>
                            <a href="../truyen/chi-tiet-truyen.html?truyen_id=${truyen.id}">${truyen.ten_truyen}</a>
                        </h3>
                        <p class="truyen-chapter">
                            <a href="../truyen/chuong.html?truyen_id=${truyen.id}&chapter_id=${latestChapter}">Độc tiếp Chapter ${latestChapter}</a>
                        </p>
                    </div>
                `;
            }).join('');
                ...
                btn.addEventListener('click', () => {
                    ...
                    .then(res => {
                        if (res.success && !res.followed) {
                            ...
                            if (!followedList.children.length) noFollowed.style.display = 'block';
                        } else {
