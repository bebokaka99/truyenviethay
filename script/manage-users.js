                ...
                data.users.forEach(user => {
                    tbody.innerHTML += `
                        <tr>
                            ...
                            <td data-status="${user.status || 'active'}">${user.status === 'active' || !user.status ? 'Active' : 'Blocked'}</td>
                            <td>
...
window.showUserDetail = function(userId) {
    fetch(`/truyenviethay/api/api.php?action=manage_users&subaction=detail&user_id=${userId}`)
        ...
        .then(data => {
            ...
            // Gọi thêm API để lấy thông tin ban_until
            fetch(`/truyenviethay/api/api.php?action=manage_users&subaction=list&page=1&search=${encodeURIComponent(user.username)}`)
                .then(res => res.json())
                .then(data => {
                    if (data.success && data.users.length > 0) {
                        const userData = data.users.find(u => u.id === userId);
                        const banUntil = userData.ban_until ? new Date(userData.ban_until).toLocaleString('vi-VN') : 'Không có';
                        document.getElementById('user-detail-content').innerHTML = `
                            <h3>${user.full_name} (@${user.username})</h3>
                            <p>Email: ${user.email}</p>
                            <p>Số điện thoại: ${user.phone || 'Chưa cập nhật'}</p>
                            <p>Thời gian khóa: ${userData.status === 'blocked' ? (userData.ban_until ? `Đến ${banUntil}` : 'Vĩnh viễn') : 'Không bị khóa'}</p>
                        `;
                        document.getElementById('user-detail-modal').style.display = 'block';
                    }
                })
                .catch(error => {
                    console.error('Lỗi khi lấy thông tin ban_until:', error);
                    document.getElementById('user-detail-content').innerHTML = `
                        <h3>${user.full_name} (@${user.username})</h3>
                        <p>Email: ${user.email}</p>
                        <p>Số điện thoại: ${user.phone || 'Chưa cập nhật'}</p>
                        <p>Thời gian khóa: Không thể tải thông tin</p>
                    `;
                    document.getElementById('user-detail-modal').style.display = 'block';
                });
        })
