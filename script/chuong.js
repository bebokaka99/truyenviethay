export function initChuong() {
  ...
  if (!truyenId || !chuongId) {
    document.querySelector(".doc-truyen-container").innerHTML =
      "<p>Truyện hoặc chương không hợp lệ.</p>";
    return;
  }
    ...
    fetch(`/truyenviethay/api/api.php?action=chuong&truyen_id=${truyenId}&chapter_id=${chuongId}`)
      ...
      document.getElementById(
        "page-title"
      ).textContent = `${data.chuong.tieu_de} - ${data.ten_truyen} - Truyenviethay`;
        ...
        data.chuong.noi_dung.replace(/\n/g, "<br>");
            ...
            nav.innerHTML = `
                ...
                <button class="nav-btn prev-btn" ${
                  data.chuong_truoc === null ? "disabled" : ""
                } data-chuong-id="${data.chuong_truoc || "null"}">
                ...
                } data-chuong-id="${data.chuong_sau || "null"}">
                ...
                <button id="follow-btn" class="nav-btn follow-btn ${
                  data.hasFollowed ? "followed" : ""
                }">
                    <i class="fas fa-bookmark"></i> ${
                      data.hasFollowed ? "Đã theo dõi" : "Theo dõi"
                    }
                </button>
            `;
      ...
      document.querySelectorAll(".prev-btn, .next-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          if (nextId !== "null")
            window.location.href = `chuong.html?truyen_id=${truyenId}&chuong_id=${nextId}`;
        });
      });
              ...
              fetch("/truyenviethay/api/api.php?action=follow", {
                ...
                .then((res) => {
                  if (res.success) {
                    ...
                    followBtn.innerHTML = `<i class="fas fa-bookmark"></i> ${
                      res.followed ? "Đã theo dõi" : "Theo dõi"
                    }`;
    ...
    .catch((error) => {
      ...
      document.querySelector(".doc-truyen-container").innerHTML =
        "<p>Lỗi khi tải chương. Vui lòng thử lại.</p>";
    });
}
