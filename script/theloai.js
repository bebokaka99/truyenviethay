export function initTheLoai() {
    ...
    fetch('/truyenviethay/api/api.php?action=theloai&subaction=categories')
        ...
        .catch(error => {
            ...
            const theloaiContainer = document.getElementById('theloai-container');
            theloaiContainer.innerHTML = '<p>Lỗi khi tải thể loại. Vui lòng thử lại.</p>';
        });
    ...
    form.querySelectorAll('input').forEach(input => {
        if (input.type === 'checkbox') {
            if (urlParams.getAll('theloai[]').includes(input.value)) input.checked = true;
    ...
    function loadTruyen(params) {
        ...
        fetch(`/truyenviethay/api/api.php?action=theloai&${query}`)
            ...
            .then(data => {
                ...
                truyenList.innerHTML = data.data.length ? '' : '<p>Không tìm thấy truyện phù hợp với bộ lọc.</p>';
                data.data.forEach(truyen => {
                    truyenList.innerHTML += `
                        <div class="khoi-truyen">
                            <a href="chi-tiet-truyen.html?id=${truyen.id}">
                ...
                if (current_page > 1) {
                    ...
                    pagination.innerHTML += `<a href="the-loai.html?${new URLSearchParams(params)}" class="page-link">«</a>`;
                }
                for (let i = 1; i <= total_pages; i++) {
                    params.page = i;
                    pagination.innerHTML += `<a href="the-loai.html?${new URLSearchParams(params)}" class="page-link ${i === current_page ? 'active' : ''}">${i}</a>`;
                }
    ...
    form.addEventListener('submit', (e) => {
        ...
        params.page = 1;
        window.history.pushState({}, '', `the-loai.html?${new URLSearchParams(params)}`);
        loadTruyen(params);
    });

    window.addEventListener('popstate', () => {
        const params = Object.fromEntries(new URLSearchParams(window.location.search));
        params.theloai = new URLSearchParams(window.location.search).getAll('theloai[]');
        loadTruyen(params);
        form.querySelectorAll('input').forEach(input => {
            if (input.type === 'checkbox') {
