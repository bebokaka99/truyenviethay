...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($status) {
        $sql = "UPDATE truyen_new SET trang_thai_kiem_duyet = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "si", $status, $truyen_id);
...
$sql = "SELECT t.id, t.ten_truyen, t.tac_gia, t.thoi_gian_cap_nhat, u.full_name as tac_gia_name FROM truyen_new t JOIN users_new u ON t.user_id = u.id WHERE t.trang_thai_kiem_duyet = 'cho_duyet'";
